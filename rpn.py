...
class MiddleAndRPN:
    def __init__(self, input, alpha=1.5, beta=1, sigma=3):
        ...
        #rpn
        #block1:
        temp_conv = ConvMD(2, 128, 128, 3, (2, 2), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 128, 128, 3, (1, 1), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 128, 128, 3, (1, 1), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 128, 128, 3, (1, 1), (1, 1), temp_conv)
        deconv1 = Deconv2D(128, 256, 3, (1, 1), (0, 0), temp_conv)
        ...
        temp_conv = ConvMD(2, 128, 128, 3, (1, 1), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 128, 128, 3, (1, 1), (1, 1), temp_conv)
        deconv2 = Deconv2D(128, 256, 2, (2, 2), (0, 0), temp_conv)

        #block3:
        temp_conv = ConvMD(2, 128, 256, 3, (2, 2), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 256, 256, 3, (1, 1), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 256, 256, 3, (1, 1), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 256, 256, 3, (1, 1), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 256, 256, 3, (1, 1), (1, 1), temp_conv)
        temp_conv = ConvMD(2, 256, 256, 3, (1, 1), (1, 1), temp_conv)
        deconv3 = Deconv2D(256, 256, 4, (4, 4), (0, 0), temp_conv)
...
def ConvMD(M, Cin, Cout, k, s, p, input):
    temp_p = np.array(p)
    ...
    temp_conv = tf.layers.batch_normalization(temp_conv, axis = -1)
    return tf.nn.relu(temp_conv)

def Deconv2D(Cin, Cout, k, s, p, input):
    temp_p = np.array(p)
    temp_p = np.lib.pad(temp_p, (1, 1), 'constant', constant_values = (0, 0))
    paddings = (np.array(temp_p)).repeat(2).reshape(4, 2)
    pad = tf.pad(input, paddings, "CONSTANT")
